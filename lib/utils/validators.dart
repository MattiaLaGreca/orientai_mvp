class Validators {
  ...
  // Pre-compiled Regex for performance (Bolt Optimization)
  static final RegExp _safeControlChars =
      RegExp(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]');
  static final RegExp _allControlChars = RegExp(r'[\x00-\x1F\x7F]');
  ...
  static String cleanMessage(String input) {
    ...
    output = output.replaceAll(_safeControlChars, '');
  ...
  static String? validateName(String? value) {
    ...
    if (value.length > 50) {
      return "Nome troppo lungo (max 50 caratteri)";
    }
    // Disallow ALL control characters including \n, \r, \t
    if (_allControlChars.hasMatch(value)) {
  ...
  static String? validateInterests(String? value) {
    if (value == null || value.trim().isEmpty) {
      return "Inserisci i tuoi interessi";
    }
    if (value.length > 500) {
      return "Testo troppo lungo (max 500 caratteri)";
    }
    return null;
  }
  ...
  /// 🔒 Sentinel Security: Validates URLs
  /// Only allows http/https schemes to prevent execution of malicious schemes (e.g., javascript:, file:).
  static bool isSafeUrl(String url) {
    try {
      final uri = Uri.parse(url);
      return uri.isScheme('http') || uri.isScheme('https');
    } catch (e) {
      // If URI parsing fails, it's unsafe.
      return false;
    }
  }
}
