...
class DatabaseService {
  ...
  Future<void> sendMessage(String text, bool isUser) async {
    ...
    // 🔒 Sentinel Security: Rate Limiting & Validation
    if (isUser) {
      if (text.length > 2000) {
  ...
  Future<List<Map<String, dynamic>>> getChatHistoryForAI(bool isPremium) async {
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Add hard limit to prevent unbounded reads and memory exhaustion
      Query query = _db
          .collection('users')
          .doc(user.uid)
          .collection('messages')
      ...
      // ⚡ Bolt Optimization: Prevent intermediate Iterable allocations via List.generate
      final docs = snapshot.docs;
      final count = docs.length;
      return List.generate(count, (index) {
        final data = docs[count - 1 - index].data() as Map<String, dynamic>;
        return {
