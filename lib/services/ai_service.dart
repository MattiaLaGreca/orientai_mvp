  ...
  Future<String> summarizeChat(bool isPremium, List<Map<String, dynamic>> chatHistory, {GenerativeModelWrapper? modelOverride}) async {
    ...
    try {
      ...
      final chatSummary = await summarizer.startChat().sendMessage(
          // 🔒 Sentinel Security: Sanitize chat history to prevent Indirect Prompt Injection.
          // Flatten newlines to prevent users from manipulating the system prompt structure.
          Content.text(chatHistory.map((entry) => "${entry['role']}: ${Validators.sanitizeForPrompt(entry['content'] ?? '')}").join("\n"))
      );
