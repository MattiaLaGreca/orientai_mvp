...
## 2026-05-28 - Defense-in-Depth Validation for Firestore & Secure Error Handling
**Vulnerability:** The application relied exclusively on UI-level validation (`OnboardingScreen`, `ProfileScreen`) for user profile data (name, interests). Direct API access or compromised clients could write arbitrarily large payloads or malformed text (Control characters) to Firestore by calling `DatabaseService.saveUserProfile`. Additionally, UI exception handling leaked raw error information using `e.toString()`.
**Learning:** Relying solely on client-side UI forms for validation exposes the backend to Denial of Wallet (DoW) via storage bloat, and risks prompt injection down the line if malformed text isn't sanitized. Furthermore, raw exception leaking exposes internal architecture details to the user.
**Prevention:** Always implement Defense in Depth: re-validate inputs (`Validators.validateName`, `Validators.validateInterests`) at the database service boundary (`DatabaseService.saveUserProfile`) before executing writes. Never directly interpolate `$e` in SnackBars; check if `e is OrientAIException` and extract the controlled `e.message` or fallback to a generic user-friendly string.
