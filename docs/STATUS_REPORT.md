...
*   **Performance:**
    ...
    *   Ottimizzazione allocazione memoria via `List.generate` per il caricamento storico.
    *   Scritture concorrenti (concurrent writes) nel database e Optimistic UI in `ChatScreen` per ridurre la latenza percepita.
