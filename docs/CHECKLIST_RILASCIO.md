...
*   [ ] **Keystore (Manuale):** Generare il file `.jks` per la firma dell'APK/AAB seguendo le istruzioni in `docs/MANUALE_RILASCIO.md`. **NON committare questo file su git.**
*   [ ] **`key.properties` (Manuale):** Configurare il file per referenziare la keystore come descritto in `docs/MANUALE_RILASCIO.md`, ed escluderlo dal version control.
