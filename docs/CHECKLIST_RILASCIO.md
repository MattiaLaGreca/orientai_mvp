...
*   [ ] **Keystore (Manuale):** Generare il file `.jks` per la firma dell'APK/AAB. **NON committare questo file su git.** (Vedi `docs/MANUALE_RILASCIO.md`)
*   [ ] **`key.properties` (Manuale):** Configurare il file per referenziare la keystore, ed escluderlo dal version control. (Vedi `docs/MANUALE_RILASCIO.md`)
