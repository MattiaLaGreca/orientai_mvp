...
*   [ ] **Keystore:** Generare manualmente il file `.jks` per la firma dell'APK/AAB (vedi [Manuale di Rilascio](MANUALE_RILASCIO.md)). **NON committare questo file su git.**
*   [ ] **`key.properties`:** Configurare manualmente il file per referenziare la keystore (vedi [Manuale di Rilascio](MANUALE_RILASCIO.md)), ed escluderlo dal version control.
