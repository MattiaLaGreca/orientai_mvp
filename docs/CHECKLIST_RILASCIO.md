...
*   [ ] **Keystore (Manuale):** Generare il file `.jks` per la firma dell'APK/AAB. **NON committare questo file su git.** Vedere `docs/MANUALE_RILASCIO.md` per le istruzioni.
*   [ ] **`key.properties` (Manuale):** Configurare il file per referenziare la keystore, ed escluderlo dal version control. Vedere `docs/MANUALE_RILASCIO.md` per le istruzioni.
